<?php
// backend/api/posts/auto_post.php
// ─────────────────────────────────────────────────────────────────────────────
// Daily Security Advisory Auto-Poster
// Normal:  https://innvikta.co.in/cybershield/api/posts/auto_post.php?secret=CyberShieldCronSecret2026
// Testing: https://innvikta.co.in/cybershield/api/posts/auto_post.php?secret=CyberShieldCronSecret2026&force=1
// ─────────────────────────────────────────────────────────────────────────────
...
$advisories = [
    [
        'title'   => '⚠️ GoldPickaxe Android Trojan Targeting Banking Apps',
    ...
    [
        'title'   => '🚨 WhatsApp Account Takeover via Verification Code Scam',
    ...
    [
        'title'   => '⚡ Juice Jacking: Public USB Charging Ports Are Dangerous',
    ...
    [
        'title'   => '🛡️ Fake Google Play System Update Installing Spyware',
    ...
    [
        'title'   => '📱 SIM Swap Fraud: Protect Your 2FA SMS Codes',
    ...
    [
        'title'   => '🌐 Man-in-the-Middle Attack via Rogue Public Wi-Fi',
    ...
    [
        'title'   => '🎣 Phishing SMS: Fake Delivery Notification Scam Active',
        'content' => "📅 " . date('Y-m-d') . "\n🔍 Source: CyberShield Threat Network\n\nAn active phishing campaign sends SMS messages pretending to be delivery services (DTDC, FedEx, India Post) with a link to 'confirm delivery'. The link leads to a fake banking page.\n\n✅ Stay Safe:\n• Do NOT click links in unexpected SMS messages\n• Go directly to the courier's official website\n• Report suspicious SMS to 1930 (Cyber Crime Helpline)",
    ],
    [
        'title'   => '🔌 Malicious Browser Extensions Stealing Passwords',
    ...
    [
        'title'   => '💳 UPI Fraud: Fake Screen Share Scam via Remote Apps',
        'content' => "📅 " . date('Y-m-d') . "\n🔍 Source: Digital Payments Safety Council\n\nFraudsters call victims claiming to be bank officials and ask them to install AnyDesk or TeamViewer. They then watch the victim enter UPI PINs and drain accounts.\n\n✅ Stay Safe:\n• Never install screen-share apps on request from strangers\n• Your bank will NEVER ask for your UPI PIN or OTP\n• Hang up and call your bank's official number to verify",
    ],
    [
        'title'   => '🤖 AI Voice Cloning Scam Targeting Families',
...
try {
    ...
    $chkAdmin = $conn->prepare("SELECT id FROM users WHERE id = 1");
    $chkAdmin->execute();
    if (!$chkAdmin->fetch()) {
        $conn->prepare("INSERT INTO users (id, username, email, password_hash, role, reputation_points, `rank`, avatar) VALUES (1, 'cybershield_admin', 'admin@innvikta.co.in', ?, 'super_admin', 9999, 'Cyber Commander', 'avatar_1')")
             ->execute([password_hash('CyberAdmin@2026', PASSWORD_BCRYPT)]);
    }

    // Force mode: pick random, skip duplicate check (for testing)
    $forceMode = isset($_GET['force']) && $_GET['force'] === '1';

    if ($forceMode) {
        $advisory = $advisories[array_rand($advisories)];
    } else {
    ...
    if (!$forceMode) {
        $chk = $conn->prepare("SELECT id FROM posts WHERE title = ? AND category_id = 9999 AND DATE(created_at) = CURDATE()");
        $chk->execute([$title]);
        if ($existing = $chk->fetch()) {
            echo json_encode([
                "status"  => "success",
                "message" => "Already published today.",
                "data"    => ["post_id" => $existing['id'], "title" => $title]
            ]);
            exit;
        }
    }

    $conn->prepare("INSERT INTO posts (user_id, category_id, title, content, is_anonymous) VALUES (1, 9999, ?, ?, 0)")->execute([$title, $content]);
    $postId = $conn->lastInsertId();

    echo json_encode([
        "status"  => "success",
        "message" => $forceMode ? "Test advisory posted!" : "Daily advisory posted!",
        "data"    => ["post_id" => $postId, "title" => $title]
    ]);
